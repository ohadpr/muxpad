...
describe('describeChanges', () => {
  ...
  it('collapses a multi-word mishearing into one pair', () => {
    ...
    expect(describeChanges('the trio go to market deck', 'the Trayo GTM deck')).toEqual([
      { from: 'trio go to market', to: 'Trayo GTM' },
    ]);
  });

  it('finds several substitutions in one message', () => {
    const changes = describeChanges(
      'check the crown schedule on Max pad for Ohio',
      'check the cron schedule on muxpad for ohados',
    );
