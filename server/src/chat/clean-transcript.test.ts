...
describe('buildCleanupPrompt', () => {
  const prompt = buildCleanupPrompt('Max pad crown schedule', ['muxpad', 'cron', 'ohados']);

  it('carries the dictated text and the glossary', () => {
    expect(prompt).toContain('Max pad crown schedule');
    expect(prompt).toContain('muxpad, cron, ohados');
  });
  ...
  it('puts the rules ahead of the glossary and the glossary ahead of the text', () => {
    expect(prompt.indexOf('Rules:')).toBeLessThan(prompt.indexOf('muxpad, cron, ohados'));
    expect(prompt.indexOf('muxpad, cron, ohados')).toBeLessThan(
...
describe('cleanTranscript', () => {
  const glossary = ['muxpad', 'cron', 'ohados', 'Trayo', 'GTM', 'artifact'];
