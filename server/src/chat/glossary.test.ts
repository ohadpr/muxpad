...
describe('isUsefulTerm', () => {
  it('keeps real names', () => {
    for (const t of ['muxpad', 'Acme', 'nimbus', 'agent-orchestration', 'GTM']) {
...
describe('buildGlossary', () => {
  ...
  const seed = () => {
    const ws = addWorkspace('Acme GTM');
  ...
  it('picks up live workspace, tab and pane names', () => {
    const { tab } = seed();
    addPane(tab, 'nimbus', '/Users/me/dev/2026/muxpad');
    const terms = buildGlossary(db, dataDir);
    expect(terms).toContain('Acme GTM');
    expect(terms).toContain('artifact lifecycle');
    expect(terms).toContain('nimbus');
  });
