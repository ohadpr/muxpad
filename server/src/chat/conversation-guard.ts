...
export async function findConversationRival(
  ...
  for (const sib of siblings) {
    ...
    // claude regex — its recorded writer is the authoritative signal, and the
    // foreground pattern also matches the runner for the reconnect window
    // where the writer has been reconciled away but the process lives on.
    if (sib.writer === 'sdk') return sib.pane_id;
    const fg = await getForegroundCommand(sib.pane_id).catch(() => null);
    if (fg && /\bclaude\b|agent-runner/i.test(fg)) return sib.pane_id;
  }
  return null;
}
