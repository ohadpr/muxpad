...
describe('PaneStore', () => {
  let panes: PaneStore;
  let workspaces: TabStore;
  let workspaceId: string;

  beforeEach(() => {
    const db = openDb(':memory:');
    panes = new PaneStore(db);
    workspaces = new TabStore(db);
    workspaceId = workspaces.create({ name: 'W', layout: 'p1' }).id;
  });

  it('creates a pane with defaults', () => {
    const p = panes.create({ tab_id: workspaceId, shell: '/bin/zsh', cwd: '/tmp' });
  ...
  it('lists panes for a workspace in creation order', () => {
    const a = panes.create({ tab_id: workspaceId, shell: '/bin/zsh', cwd: '/tmp' });
    const b = panes.create({ tab_id: workspaceId, shell: '/bin/zsh', cwd: '/tmp' });
    const list = panes.listByTab(workspaceId);
