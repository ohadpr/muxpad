...
describe('publish routes', () => {
  ...
  it('uses + persists a public_base_url hint without consulting the funnel', async () => {
    ...
    const res = await post({
      path: src,
      name: 'hinted',
      public_base_url: 'https://example-host.example-tailnet.ts.net:8443/',
    });
    expect(res.status).toBe(201);
    const body = (await res.json()) as { url: string; warning?: string };
    expect(body.url).toBe('https://example-host.example-tailnet.ts.net:8443/hinted/');
    expect(body.warning).toBeUndefined();
    ...
    expect(new GlobalsStore(db).get(PUBLIC_BASE_URL_KEY)).toBe(
      'https://example-host.example-tailnet.ts.net:8443',
    );
  });
