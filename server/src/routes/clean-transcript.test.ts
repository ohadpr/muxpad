...
describe('POST /api/clean-transcript', () => {
  ...
  it('sends the live glossary to the model', async () => {
    ...
    const model = vi.fn().mockResolvedValue('the Trayo GTM workspace is here');
    const res = await post(mount(model), { text: 'the trio go to market workspace is here' });
    expect(res.status).toBe(200);
    expect(model.mock.calls[0]?.[0]).toContain('Trayo GTM');
