...
/**
 * CSRF guard for the MAIN (tailnet) API — see docs/plans/2026-08-28-muxpad-publish.md
...
export function sameOriginGuard(opts: SameOriginOptions = {}): MiddlewareHandler {
  ...
  return async (c, next) => {
    ...
    if (origin) {
      ...
      if (origin === 'null') return refuse('opaque origin');
      const from = hostnameOf(origin);
      if (!from) return refuse('unparsable Origin');
      ...
      if (LOOPBACK.has(from) || allowed.has(from)) return next();
      const self = hostnameOf(c.req.header('host') ?? '');
      if (self && from === self) return next();
      return refuse(`Origin ${from} does not match Host ${self ?? '(none)'}`);
    }
    ...
    if (c.req.header('sec-fetch-site') === 'cross-site')
      return refuse('sec-fetch-site: cross-site, with no Origin');
