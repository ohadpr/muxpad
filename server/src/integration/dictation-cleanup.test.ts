...
const DICTATED = 'check the crown schedule on Max pad for nimble';
const CLEANED = 'check the cron schedule on muxpad for nimbus';
...
async function startInstance(): Promise<Instance> {
  ...
  const workspace = await post('/api/workspaces', { name: 'Acme GTM' });
...
describe('mobile dictation cleanup (browser)', () => {
  ...
  it('cleans the composer on tap, shows what changed, and undo restores it', async () => {
    ...
    expect(prompt).toContain('Acme GTM');
