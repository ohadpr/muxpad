...
<h3 align="center">A browser-based cockpit for terminals and coding agents, on a box you keep around.</h3>

Each browser tab is a workspace at a stable URL. Inside the workspace, named sub-tabs hold mosaic layouts of panes. A pane is a shell, a web page, or a chat with a coding agent. The PTYs and the agent sessions live on the server, not in the browser, so closing the laptop, switching to your phone, and coming back tomorrow shows the same session mid-run.

It is a personal tool. One person, one machine, no auth, no accounts, no multi-tenancy anywhere in it.

<p align="center">
  <img src="docs/hero.webp" alt="muxpad: the state rail on the left, an agent chat mid-turn on the right" width="960" />
</p>
...
<p align="center">
  <img src="docs/state-rail.webp" alt="the state rail: blocked, working, ready and idle rows" width="360" />
</p>
...
<p align="center">
  <img src="docs/demo.webp" alt="switching tabs; a row lights up as its agent starts a turn" width="900" />
</p>
...
## Memory, and things you can hand to someone
...
- **`muxpad publish`** — copies a file or directory into `~/.muxpad/public/` and prints a URL. Republishing a named slug rotates the previous copy to `/<slug>@2/` and keeps the three most recent previous versions, so updating in place never breaks the link you already sent. `--update=<slug>` refuses to *create*, which is the guard against a pile of near-identical slugs.
...
Both of the last two surface at **`/hosted`**, together, because "what am I hosting and is it up?" spans them — and separately, because an app can be stopped and an artifact cannot, and an artifact has a link worth copying and an app deliberately does not. Opening an app is a route, not a tab: closing it never touches the process.

<p align="center">
  <img src="docs/hosted.webp" alt="the Hosted view: private apps above, public artifacts below" width="900" />
</p>
...
muxpad runs on phones — iOS Safari and Chrome on Android. The navigator is the same state rail in a sheet, so "what wants me?" is one tap from anywhere.

<p align="center">
  <img src="docs/mobile.webp" alt="the navigator sheet on a phone, showing the same state rail" width="300" />
</p>
...
<p align="center">
  <img src="docs/hero-light.webp" alt="the same screen in one of the light themes" width="820" />
</p>
...
**Artifacts are public on purpose.** `muxpad publish` serves `~/.muxpad/public/` from a *second* listener on its own port — the one `tailscale funnel` is pointed at, so the funnel never touches the unauthenticated UI port. That listener carries no API, no WebSocket, no directory listings, and its root deliberately 404s. Until a funnel or tunnel is actually configured it is loopback-only, and `publish` says so instead of handing you a link that only works on your own machine. The access control on a published artifact is that its slug is unguessable (random slugs are 8 hex characters); a named slug is a URL you are choosing to hand out. Every response is sandboxed by CSP *without* `allow-same-origin`, so one artifact cannot read another's storage on the shared origin. Publishing something is publishing it — check what is in the directory.
